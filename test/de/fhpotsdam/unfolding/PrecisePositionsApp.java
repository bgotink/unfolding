...
/**
 * Three points which are very close to each other (locations) get displayed.
 * 
 * They should be visible as seperate points
 */
public class PrecisePositionsApp extends PApplet {
	...
	public void draw() {
		...
		//println("locA " + locA.x + " " + locA.y);
		//println("locB " + locB.x + " " + locB.y);

		// draw the three positions
		fill(255, 0, 0);
		ellipse(pos1[0], pos1[1], 6, 6);
		text("pos1", pos1[0], pos1[1]);

		fill(0, 255, 0);
		ellipse(pos2[0], pos2[1], 6, 6);
		text("pos2", pos2[0], pos2[1]);
		
		fill(0, 0, 255);
		ellipse(pos3[0], pos3[1], 6, 6);
		text("pos3", pos3[0], pos3[1]);

	}

	public static void main(String[] args) {
		// Here we start the actual Unfolding part
		PApplet.main(new String[] { "de.fhpotsdam.unfolding.PrecisePositionsApp" });
